...
export default function CertificateItem({
  ...
  // 🧩 Extract filename from URL if fileName not directly provided
  const displayedFileName =
    certificate.fileName ||
    (certificate.fileUrl
      ? certificate.fileUrl.split("/").pop()?.split("?")[0]
      : "No file attached")
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-grow">
        <div className="space-y-2 text-sm">
          ...
          {displayedFileName && (
            <p className="text-gray-600">
              <span className="font-medium">File:</span> {displayedFileName}
            </p>
          )}
        </div>
      ...
      <CardFooter>
        <Button
          ...
          onClick={() =>
            certificate.fileUrl && window.open(certificate.fileUrl, "_blank")
          }
